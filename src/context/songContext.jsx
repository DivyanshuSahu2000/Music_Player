...
const AppProvider = ({ children }) => {
  ...
  //     "https://api.audius.co/v1/tracks/trending?limit=20&app_name=D-Music-Player"
  //   );
  ...
  //     const formattedData = json.data.map((track) => ({
  //       id: track.id,
  //       song: track.title,
  //       singer: track.user?.name || "Unknown Artist",
  //       image: track.artwork?.["480x480"] || ["150x150"],
  //       track: `https://discoveryprovider.audius.co/v1/tracks/${track.id}/stream`,
  //     }));

  //     setDATA(formattedData);
  //     setCurrentSong(formattedData[0]); // set first song safely
  //   } catch (error) {
  ...
  const getData = async () => {
    try {
      ...
      const filtered = json.data
        ...
        .map((track) => ({
          id: track.id,
          song: limitWords(track.title, 4),
          fullTitle: track.title,
          singer: track.user?.name || "Unknown Artist",
          album: track.genre,
          music: track.mood || "Party",
          lyrics: track.tags?.split(",").slice(0, 2).join(", ") || "chill",
